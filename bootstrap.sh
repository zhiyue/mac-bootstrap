...
#   DEV_SETUP_DIR   clone target (default: ~/workspace/dev-setup)
set -euo pipefail
...
for b in /opt/homebrew/bin/brew /usr/local/bin/brew; do
  if [[ -x "$b" ]]; then brew_bin="$b"; break; fi
done
...
# 3) Core toolchain ----------------------------------------------------------
for pkg in chezmoi git gh 1password-cli; do
...
Next — once the repo is hosted AND secrets are scrubbed (dotfiles/_capture/RESTORE.md §五):
  ...
  git clone <your-repo> "$DEST"
  chezmoi init --source "$DEST/dotfiles" && chezmoi apply -v
Then finish per RESTORE.md: 1Password unlock · brew bundle Brewfile{,.fonts,.vscode} · ./install-tools.sh
EOF
  exit 0
fi

if [[ ! -d "$DEST/.git" && ! -d "$DEST/dotfiles/.git" ]]; then
  case "$DEV_SETUP_REPO" in
    https://github.com/*|git@github.com:*)
      gh auth status >/dev/null 2>&1 || { log "GitHub auth…"; gh auth login; } ;;
  esac
  log "Cloning $DEV_SETUP_REPO → $DEST"
  mkdir -p "$(dirname "$DEST")"
  git clone "$DEV_SETUP_REPO" "$DEST"
else
  ok "Repo already at $DEST"
fi

log "chezmoi init + apply (source: $DEST/dotfiles)…"
chezmoi init --source "$DEST/dotfiles"
...
$(ok "Core bootstrap done.")  Remaining manual steps (dotfiles/_capture/RESTORE.md):
  ...
  • Packages:  brew bundle --file="$DEST/dotfiles/Brewfile"{,.fonts,.vscode}
  • Tools:     ( cd "$DEST/dotfiles" && ./install-tools.sh )
EOF
